...
function baseBallPointer(array){

    // first begin by removing values with "C" and invalid points
    array.splice(array.indexOf("C")-1, 2);

    // create an array to hold all valid round points
    let validRoundPoints = [];

    // pushing values to validRoundPoints array and calculating total points 
    for (let i = 0; i < array.length; i++){
        if (array[i] === "+"){
            validRoundPoints.push(validRoundPoints[validRoundPoints.length - 1] + validRoundPoints[validRoundPoints.length - 2]);
        } else if (array[i] === "D"){
            validRoundPoints.push(validRoundPoints[validRoundPoints.length - 1] * 2)
        } else {
            validRoundPoints.push(array[i]);
        }
    }

    // calculate sum for the validRoundPoints array
    let sum = 0;

    for (let j = 0; j < validRoundPoints.length; j++){
        sum += validRoundPoints[j];
    }
